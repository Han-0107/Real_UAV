...
deque<quadrotor_msgs::PositionCommand> cmd_v;
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  //pos_cmd_pub.publish(cmd_v.front());
  /*cmd_v.push_back(cmd);
  Eigen::Vector3d f_pos(cmd_v[1].position.x,cmd_v[1].position.y,cmd_v[1].position.z);
  double dis = (odom_pos-f_pos).norm();
  if(dis<0.1){
      pos_cmd_pub.publish(cmd_v.front());
      cmd_v.pop_front();
  }*/
...
int main(int argc, char** argv) {
  ...
  //ros::Timer odm_timer = node.createTimer(ros::Duration(1), checkCallbck);
