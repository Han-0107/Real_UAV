...
void KinoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  //--offboard check--//
  mavros_mode_sub = nh.subscribe("/mavros/state",1,&KinoReplanFSM::MavmodeCallback, this);
...
void KinoReplanFSM::MavmodeCallback(const mavros_msgs::StateConstPtr &msg) {
    mode = msg->mode;
    int mm = mode.compare("OFFBOARD");
    offboard_flag = mm==0 ? true : false;
    //cout<<"--mavmode---"<<mode<<endl;
}
...
odom_pos_(2);
  ...
  else if (exec_state_ == CHECK_MAVROS_MODE)
    changeFSMExecState(GEN_NEW_TRAJ, "TRIG");
...
void KinoReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call) {
  string state_str[6] = { "INIT", "WAIT_TARGET", "GEN_NEW_TRAJ", "REPLAN_TRAJ", "EXEC_TRAJ","CHECK_MAVROS_MODE" };
  int    pre_s        = int(exec_state_);
  exec_state_         = new_state;
  cout << "[" + pos_call + "]: from " + state_str[pre_s] + " to " + state_str[int(new_state)] << endl;
  cout <<"[mavros_mode] : " + mode <<endl;
};

void KinoReplanFSM::printFSMExecState() {
  string state_str[6] = { "INIT", "WAIT_TARGET", "GEN_NEW_TRAJ", "REPLAN_TRAJ", "EXEC_TRAJ", "CHECK_MAVROS_MODE" };

  cout << "[FSM]: state: " + state_str[int(exec_state_)] << "--[mavros_mode] : " << mode << endl;
}
...
void KinoReplanFSM::execFSMCallback(const ros::TimerEvent& e) {
  ...
  switch (exec_state_) {
    ...
    case GEN_NEW_TRAJ: {
      ...
      if (success) {
        changeFSMExecState(CHECK_MAVROS_MODE, "FSM");
      } else {
        // have_target_ = false;
        // changeFSMExecState(WAIT_TARGET, "FSM");
        changeFSMExecState(GEN_NEW_TRAJ, "FSM");
    ...
    case EXEC_TRAJ: {
      ...
      if (t_cur > info->duration_ - 1e-2) {
        ...
        return;
          ...
          ros::Duration duration = cur - last;

          if(duration.toSec() >= limit)
          {
              changeFSMExecState(REPLAN_TRAJ, "FSM");
              ROS_INFO("-------------------limit is :  %lf------------------", limit);
              last = ros::Time::now();
          }
    ...
    case REPLAN_TRAJ: {
      ...
      //start_pt_ =  odom_pos_;
      //start_pt_  = info->position_traj_.evaluateDeBoorT(t_cur+0.02);
